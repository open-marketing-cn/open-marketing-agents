...
<section class="directory" aria-label="实践与工具目录">
 <div class="directory-bar"><div class="directory-tabs"><button class:active={tab==='book'} aria-pressed={tab==='book'} onclick={()=>switchTab('book')}>实践手册</button><button class:active={tab==='tools'} aria-pressed={tab==='tools'} onclick={()=>switchTab('tools')}>开源工具</button></div><label class="directory-search"><span>⌕</span><input aria-label="搜索章节或工具" placeholder="搜索问题、方法或工具" bind:value={query}/></label></div>
